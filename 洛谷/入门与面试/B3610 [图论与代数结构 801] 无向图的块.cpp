...
signed main()
{
	...
	build(n,m,0,0);
    auto syh=DBCC();
    for(int i=0;i<syh.size();++i)sort(syh[i].begin(),syh[i].end(),[](int x,int y){return x<y;});
    sort(syh.begin(),syh.end(),[](vector<int>x,vector<int>y){int len=min(x.size(),y.size());for(int i=0;i<len;++i){if(x[i]<y[i])return true;if(x[i]>y[i])return false;}return x.size()<y.size();});
	cout<<syh.size()<<'\n';
	for(auto i:syh)
    {
    	for(auto j:i)cout<<j<<" ";
