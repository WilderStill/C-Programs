...
namespace Bigint_Wilder
{
	...
	inline void operator/=(Bigint&a,long long b)
	{
		Bigint res=a/b;
		a=res;
	}
...
Bigint add_1(Bigint b)
{
    int i;
    for(i=1;b.num[i]==9;++i)b.num[i]=0;
    ++b.num[i];
...
int main()
{
	...
	while(T--)
	{
		Bigint num;
		num.input(); 
		if(num.len==1&&(num.num[1]==0||num.num[1]==1))
		{
			num.output();
			puts("");
			continue;
		}
		while(!(num.num[1]&1))num/=2;
		Bigint l=num/2,r=add_1(l),res,tp;
		res.len=tp.len=res.num[1]=tp.num[1]=1;
		while(l.len>1||l.num[1])
		{
		    if(l.num[1]&1)res+=tp;
			else tp+=res;
			l/=2;r=add_1(l);
