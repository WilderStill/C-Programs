...
#define ll int
using namespace std;
ll n,m,s,t,maxflow,mincost,dis[5011],tot=1,now[5011],hd[5011],pre[5011],flow[5011],num[5011],ave; 
bool vis[5011];
struct node
{
	ll to,nxt,val,cost;
...
int main()
{
	...
	for(int i=1;i<=n;++i)
	{
		if(num[i]<ave)addedge(s,i,ave-num[i],0);
		else if(num[i]>ave)addedge(i,t,num[i]-ave,0);
		if(i==1)addedge(1,n,INF,1),addedge(n,1,INF,1);
