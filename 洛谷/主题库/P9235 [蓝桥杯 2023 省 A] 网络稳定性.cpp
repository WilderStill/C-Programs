...
void dfs(int pos,int prefa,int val,int pt)
{
    ...
    dep[pos]=dep[prefa]+1;
    for(int i=1;i<=log2(dep[pos]);++i)fa[pos][i]=fa[fa[pos][i-1]][i-1],num[pos][i]=min(num[pos][i-1],num[fa[pos][i-1]][i-1]);
...
inline int LCA(int x,int y)
{
	...
	for(int i=maxn;~i;--i)
		if(fa[x][i]^fa[y][i])
		{
			res=min({res,num[x][i],num[y][i]});
			x=fa[x][i],y=fa[y][i];
		}	
	res=min({res,num[x][0],num[y][0]});
	return res;
}
int main()
{
