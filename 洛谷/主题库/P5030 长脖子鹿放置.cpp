...
#define ll long long
using namespace std;
ll n,m,sumk,sum,s,t,ans,dis[M],tot=1,now[M],hd[M],mp[415][415];
const pair<ll,ll>d[8]={{-1,-3},{-3,-1},{3,-1},{1,-3},{-1,3},{-3,1},{3,1},{1,3}};
struct node
{
	ll to,nxt,val;
}edge[M];
...
int main()
{
	scanf("%lld%lld%lld",&n,&m,&sumk);
	s=0,t=n*m+1;
	for(ll i=1,a,b;i<=sumk;i++)
	{
		scanf("%lld%lld",&a,&b);
		if(!mp[a][b])sum++;
		mp[a][b]=1;
