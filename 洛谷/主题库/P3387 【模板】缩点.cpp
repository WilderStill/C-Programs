...
#define M 1145141
using namespace std;
int n,m,val[M],u[M],v[M],head1[M],head2[M],dfn[M],low[M],scc[M],scclen[M],dp[M],ans,sccnum,tot,tim,cnt;
stack<int>st;
bool inst[M];
struct edge
{
...
void dfs(int x)
{
	...
	{
		if(!dp[edge2[i].to])dfs(edge2[i].to);
		maxn=max(maxn,dp[edge2[i].to]);
	}
	dp[x]+=maxn;
}
int main()
{
	...
	for(int i=1;i<=sccnum;i++)if(!dp[i])dfs(i);
	for(int i=1;i<=sccnum;i++)ans=max(ans,dp[i]);
	printf("%d",ans);
	return 0;
}
