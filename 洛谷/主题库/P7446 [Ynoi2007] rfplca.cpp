...
int n,m,blen,tot,lastans,L[400010],R[400010],blo[400010],cnt[400010],pa[400010],fa[400010];
ll mk[400010];
...
int main()
{
	...
	for(int i=1;i<=m;++i)
	{
		int op=read(),l=read()^lastans,r=read()^lastans;
		if(op==1)
		{
			int x=read()^lastans;
			modify(l,r,x);
		}
		else write(lastans=query(l,r)),puts("");
	}
	return 0;
}
