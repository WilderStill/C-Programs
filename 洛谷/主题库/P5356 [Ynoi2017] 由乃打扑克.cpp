...
int n,m,blen,tot,blo[M],L[M],R[M];
ll mx,mn,num[M],mk[M];
vector<int>v[M];
...
inline int query(int pl,int pr,ll k)
{
	int l=blo[pl],r=blo[pr],res=0;
	if(l==r)
	{
		for(int i=pl;i<=pr;++i)if(num[i]+mk[blo[i]]<=k)++res;
		return res;
	}
	for(int i=pl;i<=R[l];++i)if(num[i]+mk[blo[i]]<=k)++res;
	for(int i=l+1;i<r;++i)
	{
		...
		res+=(upper_bound(v[i].begin(),v[i].end(),k-mk[i])-v[i].begin());
	}
	for(int i=L[r];i<=pr;++i)if(num[i]+mk[blo[i]]<=k)++res;
	return res;
}
int main()
{
	...
	for(int i=1,op,l,r,k;i<=m;++i)
	{
		...
		if(op==1)
		{
			...
			ll tl=mn,tr=mx,ans=0;
			while(tl<=tr)
			{
				ll mid=tl+tr>>1;
				if(query(l,r,mid)<k)tl=mid+1;
