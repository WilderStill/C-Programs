...
void MAX_SUM(int v1,int v2)
{
	int x,y,z;
	splits(rt,v1-1,x,y);
	splits(y,v2-v1+1,y,z);
	printf("%lld\n",tree[y].max);
...
int main()
{
	...
	for(int i=1,v1,v2;i<=m;++i)
	{
		string op;
		cin>>op;
		if(op=="I")scanf("%d",&v1),INSERT(v1-1,1);
		else if(op=="D")scanf("%d",&v1),DELETE(v1,1);
		else if(op=="R")scanf("%d%d",&v1,&v2),MAKE_SAME(v1,1,v2);
		else if(op=="Q")scanf("%d%d",&v1,&v2),MAX_SUM(v1,v2);
	}
	return 0;
}
